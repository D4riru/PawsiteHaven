...
import { BirthdayPicker, Button, Input, StepProgress } from '@/app/components';
...
export default function SignupStep2() {
  ...
  const handleNext = async (e: React.FormEvent) => {
    ...
    } else if (!/^\+?1?\d{9,}/.test(phone.replace(/\D/g, ''))) {
    ...
    if (!birthday.trim()) {
      ...
      if (Number.isNaN(parsed.getTime())) {
        newErrors.birthday = 'Please choose a valid date';
      } else {
    ...
    try {
      ...
      sessionStorage.setItem('signupBirthday', birthday);
      setTimeout(() => {
        setLoading(false);
  ...
  return (
    <SignupShell>
      ...
      <StepProgress steps={SIGNUP_PROGRESS_STEPS} currentStep={2} />
